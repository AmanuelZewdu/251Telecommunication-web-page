...
//               <div
//                 key={member.id}
//                 className="flex flex-col gap-2 items-center p-0 m-0 cursor-pointer w-36 sm:w-44 md:w-64"
...
//                   src={member.imgSrc}
//                   alt={member.name}
//                   className="h-32 w-full sm:h-44 md:h-64 object-cover rounded-md transition-transform duration-300 ease-in-out hover:scale-105"
//                 />
...
const Abouthero = () => {
  ...
  return (
    <section className="about-wrapper">
      <Expermental />
      <MissionVisionValues />
      <div className="about-container p-4 md:px-10 lg:px-20">
        <div className="team-container">
          {!selectedMember && (
            <div className="flex justify-center items-center text-center mb-4">
              <h1 className="text-lg sm:text-xl md:text-2xl lg:text-3xl text-white font-light" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                OUR TEAM, OUR GREATEST ASSET
              </h1>
            </div>
          )}

          <div className="image-container flex flex-wrap justify-center gap-2 md:gap-4">
            {teamMembers.map((member) => (
              <div
                key={member.id}
                className="flex flex-col items-center cursor-pointer w-28 sm:w-36 md:w-44 lg:w-56"
                onMouseEnter={() => handleMouseEnter(member)}
                onMouseLeave={handleMouseLeave}
              >
                <img
                  src={member.imgSrc}
                  alt={member.name}
                  className="h-24 sm:h-32 md:h-40 lg:h-56 w-full object-cover rounded-md transition-transform duration-300 ease-in-out hover:scale-105"
                />
                <span className="text-white text-xs sm:text-sm md:text-base lg:text-lg font-light text-center">
                  {member.name.split(" ")[0]} <br /> {member.name.split(" ").slice(1).join(" ")}
                </span>
              </div>
            ))}
          ...
          {selectedMember && (
            <div className="w-full p-2 md:px-6 lg:px-10 flex justify-center items-center bg-black/70 mt-4 rounded-md">
              <div className="bg-black relative border border-white flex flex-col md:flex-row gap-2 md:gap-4 p-2 md:p-4 lg:p-6 rounded-lg max-w-3xl selected-member-container">
                <div>
                  <h2 className="text-base sm:text-lg md:text-xl lg:text-2xl text-yellow-500 font-bold">{selectedMember.name}</h2>
                  <p className="text-xs sm:text-sm md:text-base text-white">{selectedMember.role}</p>
                </div>
                <div className="border-b md:border-r md:border-b-0 border-white my-1 md:my-0"></div>
                <p className="text-xs sm:text-sm md:text-base text-white">{selectedMember.description}</p>
                <button onClick={handleMouseLeave} className="text-white absolute top-1 right-1 p-1 px-2 bg-red-600 rounded-full">
                  x
                </button>
              </div>
